...
namespace InventoryTests
{
    ...
    public class WarehouseTests
    {
        ...
        [Case("Долгий тест (для проверки параллелизма)")]
        public async Task LongTest1()
        {
            await Task.Delay(1000); // Имитация долгой работы
            Verify.ThatTrue(true);
        }

        [Case("Еще один долгий тест")]
        public async Task LongTest2()
        {
            await Task.Delay(1000);
            Verify.ThatTrue(true);
        }

        [Case("Тест с тайм-аутом (должен упасть)")]
        [Timeout(500)] // Ограничение 0.5 сек
        public async Task TimeoutFailTest()
        {
            await Task.Delay(2000); // Спит 2 сек
            Verify.ThatTrue(true);
        }
